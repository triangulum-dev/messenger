...
class Controller {
    constructor(id, source) {
        ...
        _Controller_onConnect.set(this, (connection) => {
            ...
            (0, utils_js_1.addMessageEventListener)(connection.port, onMessage);
        ...
        __classPrivateFieldSet(this, _Controller_listenRef, connection_js_1.Connection.listen(this.id, this.source, __classPrivateFieldGet(this, _Controller_onConnect, "f")), "f");
    }
    onPromise(handler) {
    ...
    close() {
        ...
        while (__classPrivateFieldGet(this, _Controller_activeRequests, "f").length > 0) {
            ...
            if (!req)
                break;
            if (req.type === 'promise') {
                req.connection.port.postMessage({ type: messages_js_1.MessageType.Reject, id: req.id, error: new Error("Connection closed") });
            }
            else if (req.type === 'observable') {
                req.connection.port.postMessage({ type: messages_js_1.MessageType.Error, id: req.id, error: new Error("Connection closed") });
            }
        }
...
_Controller_listenRef = new WeakMap(), _Controller_connections = new WeakMap(), _Controller_promiseCallback = new WeakMap(), _Controller_observableCallback = new WeakMap(), _Controller_activeRequests = new WeakMap(), _Controller_onConnect = new WeakMap(), _Controller_onMessage = new WeakMap(), _Controller_instances = new WeakSet(), _Controller_handlePromiseMessage = async function _Controller_handlePromiseMessage(connection, data) {
    ...
    try {
        ...
        req.connection.port.postMessage((0, messages_js_1.resolveMessage)(req.id, result));
    }
    catch (error) {
        try {
            req.connection.port.postMessage((0, messages_js_1.rejectMessage)(req.id, error));
        }
        catch (e) {
    ...
    const observer = {
        ...
        complete: () => {
            if (!completed) {
                completed = true;
                connection.port.postMessage({ type: messages_js_1.MessageType.Complete, id });
                removeFromActiveRequests();
            }
        },
    };
